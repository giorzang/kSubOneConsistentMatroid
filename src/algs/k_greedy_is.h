// algs/k_greedy_is.h
#ifndef K_GREEDY_IS_H
#define K_GREEDY_IS_H
...
inline void build_random_max_independent_set_V1_is(
    ...
    if (cap.size() != p) {
        throw std::invalid_argument("kGreedyIS: cap.size() != derive_p(g)");
    }
...
inline Result run_k_greedy_is(
    ...
    if (u_upper.size() != K) {
        throw std::invalid_argument("kGreedyIS: u_upper.size() != g.K");
    }

    std::vector<mygraph::node_id> V1;
    std::vector<std::uint8_t> inV1;
    build_random_max_independent_set_V1_is(g, cap, V1, inV1);
    ...
    auto has_remaining_quota = [&]() -> bool {
        for (std::size_t i = 1; i <= K; ++i) {
            if (cnt_lbl[i] < u_upper[i - 1]) return true;
        }
        return false;
    };

    while (has_remaining_quota()) {
        ...
        for (mygraph::node_id u : V1) {
            ...
            for (ksub::Label lbl = 1; lbl <= static_cast<ksub::Label>(K); ++lbl) {
                ...
                if (cnt_lbl[li] >= u_upper[li - 1]) continue; // quota full
        ...
        const std::size_t li = static_cast<std::size_t>(best_lbl);

        if (bu >= n) break;
        if (best_lbl == 0 || li > K) break;
