// preprocess_sensor.cpp
// Partition-aware, FORMAT A ONLY: orig_node_id part_id
// + Renumber node_id by FIRST appearance in edges.txt
// + Compress part_id to 0..p-1 by ascending raw part_id
// + Read entropy.txt: orig_node_id H1..HK and store into g.nodes[new_id].weights[t]
#include <iostream>
#include <string>
#include <cstdlib>
#include <fstream>
...
static inline double sanitize_entropy(double h) {
    if (!std::isfinite(h)) return 0.0;
    return (h >= 0.0) ? h : 0.0; // entropy should be >= 0
}
...
static void print_usage(const char* prog) {
    std::cerr
        << "Usage:\n"
        << "  " << prog
        << " edges.txt parts.txt entropy.txt output.bin K [undirected(0/1)=1] [randomize_alpha(0/1)=0] [seed=42]\n\n"
        << "edges.txt format:\n"
        << "  - Moi dong: u v [w]\n"
        << "    + u, v: so nguyen khong am (ID goc)\n"
        ...
        << "  - Strict: moi node trong edges.txt PHAI co part_id; parts.txt thua node -> loi.\n\n"
        << "entropy.txt format (BAT BUOC):\n"
        << "  - Moi dong: orig_node_id H1 H2 ... HK\n"
        << "  - Bat buoc DUNG (K+1) cot; thua/thiieu -> loi\n"
        << "  - Strict: moi node trong edges.txt PHAI co entropy; entropy.txt thua node -> loi.\n\n"
        << "Renumber rule (node id):\n"
        << "  - ID goc trong edges.txt map sang node_id 0..n-1 theo THU TU XUAT HIEN LAN DAU.\n"
        << "Renumber rule (part id):\n"
        << "  - part_id doc tu parts.txt se duoc compress ve 0..p-1 theo THU TU TANG DAN raw part_id.\n";
}
...
int main(int argc, char** argv) {
    ...
    const string entropy_txt = argv[3];
    const string output_bin  = argv[4];
    const size_t K = static_cast<size_t>(std::strtoull(argv[5], nullptr, 10));

    bool undirected = true;
    bool randomize_alpha = false;
    unsigned seed = 42;

    if (argc >= 7) undirected = (std::atoi(argv[6]) != 0);
    if (argc >= 8) randomize_alpha = (std::atoi(argv[7]) != 0);
    if (argc >= 9) seed = static_cast<unsigned>(std::strtoul(argv[8], nullptr, 10));

    cout << "Preprocess SENSOR-ENTROPY (FORMAT A ONLY, part_id will be renumbered):\n";
    cout << "  edges.txt        = " << edges_txt << "\n";
    cout << "  parts.txt        = " << parts_txt << "\n";
    cout << "  entropy.txt      = " << entropy_txt << "\n";
    cout << "  output.bin       = " << output_bin << "\n";
    cout << "  K                = " << K << "\n";
    cout << "  undirected       = " << (undirected ? "true" : "false") << "\n";
    cout << "  randomize_alpha  = " << (randomize_alpha ? "true" : "false") << "\n";
    cout << "  seed             = " << seed << "\n";
    ...
    // --------- 3) Read entropy.txt ----------
    ifstream fe(entropy_txt);
    if (!fe) {
        cerr << "Error: cannot open entropy file: " << entropy_txt << "\n";
        return 1;
    }

    unordered_map<uint64_t, vector<double>> ent_map; // orig -> [H1..HK]
    ent_map.reserve(n * 2);

    size_t ent_line_no = 0;
    while (std::getline(fe, line)) {
        ...
        if (!(ss >> a)) {
            cerr << "Error: entropy.txt invalid format at line " << ent_line_no
                 << " (expected: orig_node_id H1..HK)\n";
            return 1;
        }

        const uint64_t orig = static_cast<uint64_t>(a);
        if (ent_map.find(orig) != ent_map.end()) {
            cerr << "Error: entropy.txt duplicate orig_node_id = " << orig
                 << " at line " << ent_line_no << "\n";
            return 1;
        }
        ...
        for (size_t t = 0; t < K; ++t) {
            double h = 0.0;
            if (!(ss >> h)) {
                cerr << "Error: entropy.txt missing H" << (t + 1)
                     << " at line " << ent_line_no
                     << " (need exactly K values)\n";
                return 1;
            }
            H[t] = sanitize_entropy(h);
        }

        // no extra tokens allowed
        std::string extra;
        if (ss >> extra) {
            cerr << "Error: entropy.txt has extra token at line " << ent_line_no
                 << " (expected exactly K+1 columns)\n";
            return 1;
        }

        ent_map.emplace(orig, std::move(H));
    }
    fe.close();

    if (ent_map.empty()) {
        cerr << "Error: entropy.txt has no valid data lines.\n";
        return 1;
    }

    // --------- 4) Strict consistency checks ----------
    ...
    // (iii) Every orig id in entropy must appear in edges
    for (const auto& kv : ent_map) {
        const uint64_t orig = kv.first;
        if (idmap.find(orig) == idmap.end()) {
            cerr << "Error: entropy.txt contains orig_node_id = " << orig
                 << " but this node never appears in edges.txt\n";
            return 1;
        }
    }

    // (iv) Every node appearing in edges must have entropy
    if (ent_map.size() != idmap.size()) {
        for (const auto& kv : idmap) {
            const uint64_t orig = kv.first;
            if (ent_map.find(orig) == ent_map.end()) {
                cerr << "Error: missing entropy vector for orig_node_id = " << orig
                     << " (node appears in edges.txt but not in entropy.txt)\n";
                return 1;
            }
        }
        cerr << "Error: mismatch between edges nodes and entropy nodes.\n";
        return 1;
    }

    cout << "  parts assigned     = " << part_map_orig.size() << " / " << n << "\n";
    cout << "  entropy assigned   = " << ent_map.size()      << " / " << n << "\n";

    // --------- 5) Renumber part_id to 0..p-1 (compress) ----------
    ...
    // nodes weights: entropy
    for (const auto& kv : ent_map) {
        const uint64_t orig = kv.first;
        const node_id u = idmap[orig];
        const auto& H = kv.second; // size K
        for (size_t t = 0; t < K; ++t) {
            g.nodes[u].weights[t] = H[t];
        }
    }

    // alpha: not used by sensor_entropy objective; keep 1.0 or randomize if requested
    if (randomize_alpha) {
    ...
    for (const auto& re : rawEdges) {
        ...
        E.u = u;
        E.v = v;
        E.weights.assign(K, re.w); // irrelevant for sensor_entropy but keep consistent
